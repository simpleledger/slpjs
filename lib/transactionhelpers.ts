...
export interface scriptSigP2SH {
    ...
    unlockingScriptBufArray: Buffer|(number|Buffer)[]; 
}
...
export class TransactionHelpers {
    ...
    build_P2SH_multisig_redeem_data(m: number, pubKeys: string[]|Buffer[]): multisigRedeemData {
        ...
        let addr = this.slp.BITBOX.Address.fromOutputScript(this.slp.BITBOX.Script.scriptHash.output.encode(this.slp.BITBOX.Crypto.hash160(redeemScript)))
    ...
    build_P2SH_multisig_scriptSig(redeemData: multisigRedeemData, input_index: number, sigs: InputSigData[]): scriptSigP2SH {
        ...
        let orderedSigs = pubKeysHex.map(pub => {
            ...
            console.log("sig index", sig!.index)
            console.log("sig", sig!.signatureBuf.toString('hex'))
            return sig!.signatureBuf;
        })

        console.log(orderedSigs);

        // build the unlocking script for multisig p2sh

        let unlockingScript = this.slp.BITBOX.Script.encodeP2MSInput(orderedSigs) //[ 0x00, ...orderedSigs ];
